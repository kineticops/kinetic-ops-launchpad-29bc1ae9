...
const CHAT_MESSAGES = [
  ...
  { from: "bot" as const, text: "✅ ¡Cita confirmada!\n\n📅 Jueves 3 abril — 17:30h\n🦷 Limpieza dental\n📍 Clínica Dental Sonríe\n\n📅 Añadida a Google Calendar\n🔔 Recordatorio 24h antes\n\n¡Hasta entonces! 😊", time: "10:33" },
];

const STATS = [
  { value: "90%", desc: "Tasa de lectura en WhatsApp vs 20% en email.", icon: "✅" },
  { value: "76%", desc: "De adultos prefieren contactar por mensajería.", icon: "✅" },
  { value: "#1", desc: "Canal preferido para gestiones rápidas en España.", icon: "✅" },
  { value: "84%", desc: "Del uso del móvil es en apps de mensajería.", icon: "✅" },
];
    ...
    <section className="section-padding section-gray">
      <div ref={ref} className="fade-section max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-desc max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start mb-16">
          <div className="flex flex-col items-center gap-4">
            <div className="bg-accent/10 border border-accent/20 rounded-full px-4 py-1.5 text-accent text-xs font-bold">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {STATS.map((s, i) => (
            <div key={i} className="bg-card border border-border rounded-2xl p-6 text-center hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
              <span className="text-navy text-3xl sm:text-4xl font-extrabold block mb-2">{s.value}</span>
              <span className="text-gray-desc text-xs leading-relaxed">{s.desc}</span>
            </div>
          ))}
        </div>
      </div>
